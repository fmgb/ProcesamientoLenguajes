...
class TraductorDR {
        ...
        if(token.tipo == Token.ID || token.tipo == Token.NENTERO || token.tipo == Token.NREAL || token.tipo == Token.PARI)
            {
                ...
                Atributos resultadoTerm_prima = Term_prima(atributosAux,token);
                if(resultadoTerm_prima.th.equals("div"))
                    {
                        /*System.out.println("DIVERSION");
                        System.out.println(resultadoFactor.tipo);
                        System.out.println(resultadoTerm_prima.tipo);
                        */
    ...
    public final Atributos  Term_prima(Atributos atributos, Token tokencito) // 33 Term_prima -> mulop Factor Term_prima
                                                           // 34 Term_prima -> €
    {
        Atributos resultado = new Atributos();
        Atributos atributosAux = new Atributos();
